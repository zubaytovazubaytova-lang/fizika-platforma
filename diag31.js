...
(async () => {
  ...
  const email = `bikecheck31_${Date.now()}@test.local`;
  ...
  const solveBtn = page.locator('button').filter({ hasText: /Yeching/i }).first();
  console.log('yeching enabled?', await solveBtn.isEnabled().catch(()=> '?'));
  ...
  // locate the bike with a wide screenshot first
  await canvas.screenshot({ path: 'd31_wide.png' });

  // moderate zoom — fewer wheel steps than diag29 (which overshot)
  ...
  await canvas.screenshot({ path: 'd31_zoomed.png' });

  for (let i=0;i<8;i++){
    await page.screenshot({ path: `d31_wheel${i}.png`,
      clip: { x: box.x + cw*0.05, y: box.y + ch*0.55, width: cw*0.45, height: ch*0.40 } });
