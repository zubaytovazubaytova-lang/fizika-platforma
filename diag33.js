...
(async () => {
  ...
  const email = `bikecheck33_${Date.now()}@test.local`;
  ...
  // Step 1: locate bike precisely with grid overlay-ish full shot at default zoom
  await canvas.screenshot({ path: 'd33_locate.png' });

  // Step 2: zoom moderately (4 steps) centered roughly where bike was in d31 (~0.215, 0.715)
  const tx = box.x + cw*0.215, ty = box.y + ch*0.715;
  await page.mouse.move(tx, ty);
  for (let i=0;i<4;i++){ await page.mouse.wheel(0, -100); await page.waitForTimeout(150); }
  await page.waitForTimeout(300);
  await canvas.screenshot({ path: 'd33_afterzoom.png' });
