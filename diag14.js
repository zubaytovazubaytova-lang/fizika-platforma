...
(async () => {
  ...
  const email = `bikecheck6_${Date.now()}@test.local`;
  ...
  // very tight crop on rear-wheel area (per d13: lower-left blob ~ rel x .305 y .815 of canvas-box)
  await page.screenshot({ path: 'd14_wheel1.png', clip: { x: box.x + cw*0.295, y: box.y + ch*0.795, width: cw*0.05, height: ch*0.06 } });
  // and front-wheel arc (right one ~ rel x .335 y .79)
  await page.screenshot({ path: 'd14_wheel2.png', clip: { x: box.x + cw*0.32, y: box.y + ch*0.775, width: cw*0.05, height: ch*0.06 } });
  // whole bike + ample margin
  await page.screenshot({ path: 'd14_wide.png', clip: { x: box.x + cw*0.27, y: box.y + ch*0.70, width: cw*0.13, height: ch*0.18 } });
