...
(async () => {
  ...
  const email = `bikecheck12_${Date.now()}@test.local`;
  ...
  console.log('start count', await startBtn.count(), 'enabled', await startBtn.isEnabled().catch(()=>'?'));
  ...
  for (let step = 0; step < 6; step++) {
    ...
    await canvas.screenshot({ path: `d20_orbit${step}.png` });
  }
  await browser.close();
