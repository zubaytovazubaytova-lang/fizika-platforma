...
(async () => {
  ...
  const email = `bikecheck14_${Date.now()}@test.local`;
  ...
  // Reset first to get to a known stationary state at flag A
  ...
  } else if (await pauseBtn.count()) {
    console.log('already running -> pausing directly');
    await pauseBtn.click();
  }
  await page.waitForTimeout(500);
  await canvas.screenshot({ path: 'd22_full.png' });
  ...
  for (let step = 0; step < 6; step++) {
    ...
    await canvas.screenshot({ path: `d22_orbit${step}.png` });
  }
  await browser.close();
