...
(async () => {
  ...
  const email = `bikecheck4_${Date.now()}@test.local`;
  ...
  await canvas.screenshot({ path: 'd12_full.png' });
  // bike now near rel (0.27, 0.79) of canvas per d11_step5 -- crop tight
  await page.screenshot({ path: 'd12_crop.png',
    clip: { x: box.x + cw*0.22, y: box.y + ch*0.68, width: cw*0.16, height: ch*0.22 } });
