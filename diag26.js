...
(async () => {
  ...
  const email = `bikecheck18_${Date.now()}@test.local`;
  ...
  // Click "Yeching" to open the solve panel
  const yechBtn = page.locator('button').filter({ hasText: /Yeching/i }).first();
  console.log('yeching count', await yechBtn.count());
  await yechBtn.click();
  await page.waitForTimeout(1000);
  const txt = await page.locator('body').innerText();
  console.log(txt.slice(0, 2000));
  await page.screenshot({ path: 'd26_solve.png' });
  await browser.close();
