...
(async () => {
  ...
  // zoom in moderately on bike start position
  ...
  await canvas.screenshot({ path: 'd3_t0.png' });
  const startBtn = page.locator('button').filter({ hasText: /Start/i }).first();
  if (await startBtn.count()) { await startBtn.click(); }
  await page.waitForTimeout(550);
  await canvas.screenshot({ path: 'd3_t1.png' });
  await page.waitForTimeout(550);
  await canvas.screenshot({ path: 'd3_t2.png' });
  await browser.close();
