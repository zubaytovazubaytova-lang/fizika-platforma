...
(async () => {
  ...
  const email = `bikecheck16_${Date.now()}@test.local`;
  ...
  for (let i=0;i<4;i++){ await page.mouse.wheel(0, -100); await page.waitForTimeout(180); await canvas.screenshot({ path: `d24_step${i}.png` }); }
  await browser.close();
