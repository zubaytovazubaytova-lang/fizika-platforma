...
(async () => {
  ...
  const email = `bikecheck36_${Date.now()}@test.local`;
  ...
  // Approach: ZOOM IN FIRST while stationary (bike sits still at flag A before solving), centered precisely.
  // From d33_locate.png (no zoom yet) bike was at rel ~ (0.196, 0.703).
  const tx = box.x + cw*0.196, ty = box.y + ch*0.703;
  await page.mouse.move(tx, ty);
  for (let i=0;i<8;i++){ await page.mouse.wheel(0, -70); await page.waitForTimeout(160); }
  await page.waitForTimeout(400);
  await canvas.screenshot({ path: 'd36_prezoom_full.png' });
